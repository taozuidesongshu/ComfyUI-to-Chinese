	...
	init() {
		...
		const addCustomColorPalette = async (colorPalette) => {
			if (typeof(colorPalette) !== "object") {
				app.ui.dialog.show("主题无效");
				return;
			}

			if (!colorPalette.id) {
				app.ui.dialog.show("主题缺少ID");
				return;
			}

			if (!colorPalette.name) {
				app.ui.dialog.show("主题缺少名字");
				return;
			}

			if (!colorPalette.colors) {
				app.ui.dialog.show("主题缺少颜色");
				return;
			}

			if (colorPalette.colors.node_slot && typeof(colorPalette.colors.node_slot) !== "object") {
				app.ui.dialog.show("无效的主题颜色节点插槽");
				return;
			}
