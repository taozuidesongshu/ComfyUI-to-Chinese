...
export class ComfyUI {
	constructor(app) {
		...
		this.queue = new ComfyList("队列");
		this.history = new ComfyList("历史");
	...
	setStatus(status) {
		this.queueSize.textContent = "队列数量: " + (status ? status.exec_info.queue_remaining : "ERR");
		if (status) {
			if (
				this.lastQueueSize != 0 &&
